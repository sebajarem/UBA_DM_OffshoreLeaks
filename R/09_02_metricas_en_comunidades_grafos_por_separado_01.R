...
# saco el grafo 1 que es grade y ya esta calculado
indices_lista_no_nulos = c(indices_lista_no_nulos[2:length(indices_lista_no_nulos)])
...
for (indice in indices_lista_no_nulos) {
  ...
  if (length(V(lista_datos_grafo$g)) > 1 ) {
    ...
    print(paste("cantidad de grafos totales ",length(indices_lista_no_nulos)))
    
  }else{
    print(paste("grafo número",indice,"es de 1 solo nodo"))
    print(paste("cantidad de grafos",length(indices_lista_no_nulos)))
    
  }
  
  saveRDS(lista_datos_grafo, glue("./salidas_datos/lista_de_grafos/grafo_{indice}.rds"))
